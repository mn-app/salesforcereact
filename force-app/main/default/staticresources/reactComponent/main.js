...
var App = function (_Component) {
	...
	_createClass(App, [{
		...
		value: function fetchData() {
			...
			fetch('https://api.npoint.io/30855e03c0357e27a1c5').then(function (response) {
				if (response.ok) {
					return response.json();
				} else {
					_this2.setState({ fetchedData: 'Loading error:' + response.statusText });
				}
			}).then(function (json) {
				return _this2.setState({ fetchedData: JSON.stringify(json) });
			}).catch(function (error) {
				return _this2.setState({ fetchedData: error.message });
			});
		}
	}, {
		key: 'fetchDataApex',
		value: function fetchDataApex() {
			var _this3 = this;

			this.setState({ fetchedDataApex: 'start fetching...' });
			_lightningContainer2.default.callApex('ApexFetch.ff', function (result) {
				console.log('result', result);
				_this3.setState({ fetchedDataApex: result.replace(/&quot;/g, '"') });
			}, { escape: true });
		}
	}, {
		key: 'render',
		value: function render() {
			return _react2.default.createElement(
				...
				_react2.default.createElement(
					...
					_react2.default.createElement(
						'button',
						{ onClick: this.fetchDataApex.bind(this) },
						'Fetch data (Apex)'
					),
